...
src/main/resources/application.properties                        # NEW — EMPTY presence marker (Quarkus requires base file before profile files load)
...
presence marker — VERIFIED: Quarkus ignores `application-{profile}.properties`
unless a base `application.properties` exists in the same location. ALL values
